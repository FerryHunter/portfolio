...
/**
 * Case study: Investment App Reimagined (studi konsep, bukan klien).
 *
 * Susunan section-nya diturunkan dari NanovestInvestmentApp — bentuk
 * data-nya identik persis — minus section "mockup" komposit, sama
 * seperti Amazon: case ini cuma punya satu cover + empat tangkapan
 * layar utuh, jadi galerinya satu grid rata (bukan split 4+4).
 *
 * Pasangannya di preview statis adalah
 * scripts/case_investment_app_reimagined.py.
 */
...
export default function InvestmentAppReimagined({
  dict,
  lang,
  data,
}: CaseProps<Cases["investment-app-reimagined"]>) {
  const home = `/${lang}`;
  ...
  return (
    <>
      ...
      {/* Tidak ada rekaman untuk case ini, jadi section-nya dilewatkan
          sama sekali daripada tampil sebagai placeholder kosong — sama
          seperti case Amazon. Isi `video.src` di content/*.json begitu
          rekamannya ada, section ini muncul kembali otomatis. */}
      {data.video.src ? (
