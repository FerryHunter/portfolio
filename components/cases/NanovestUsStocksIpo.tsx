...
/**
 * Case study: Nanovest · US Stocks IPO.
 *
 * Satu komponen untuk satu proyek. Susunan section-nya sejajar
 * dengan NanovestLimitOrder (satu pola untuk case Nanovest lain),
 * dan seluruh asetnya layar ponsel potret 750 × 1624 — tidak ada
 * satu pun gambar yang dipotong, yang dibatasi lebarnya (lihat
 * <Media>).
 *
 * Pasangannya di preview statis adalah
 * scripts/case_nanovest_us_stocks_ipo.py — dua file yang harus
...
export default function NanovestUsStocksIpo({
  dict,
  lang,
  data,
}: CaseProps<Cases["nanovest-us-stocks-ipo"]>) {
  const home = `/${lang}`;
  ...
  return (
    <>
      ...
      {/* Tanpa section--divided: hairline-nya bertabrakan dengan
          lengkung bawah dua ponsel hero di atasnya. */}
      ...
      {/* §6.4 — markup showreel. Rekamannya belum ada, jadi seluruh
          section disembunyikan dulu (bukan ditampilkan sebagai
          placeholder kosong) sampai data.video.src terisi. */}
      {data.video.src ? (
