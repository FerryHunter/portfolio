...
/**
 * Case study: Rampage Quest · Daily Quests HUD.
 *
 * Susunan section-nya sejajar dengan NanovestLimitOrder (hero →
 * overview → scope → video → blocks → gallery → metrics →
 * deliverables → next), tapi asetnya tangkapan layar landscape
 * 3120 × 1440 (HUD game), bukan potret ponsel — jadi <Media> memakai
 * slot `.case-media` biasa, bukan varian `--phone`.
 *
 * Pasangannya di preview statis adalah
 * scripts/case_game_quest.py — dua file yang harus tetap seragam,
 * mudah dibaca berdampingan.
 */
...
export default function GameQuest({
  dict,
  lang,
  data,
}: CaseProps<Cases["game-quest"]>) {
  const home = `/${lang}`;

  return (
    <>
      <section className="case-hero" id="top">
        <div className="case-hero-deco case-hero-deco--game-quest" aria-hidden="true">
