...
/**
 * §6.11 — Case study: Nanovest · Investment App.
 *
 * Sepasang dengan scripts/case_nanovest_investment_app.py. Susunan
 * section-nya sama dengan NanovestCalendar (satu pola untuk dua
 * proyek Nanovest). Hero memakai komposit lanskap dua ponsel
 * (--shot, max-width 1120px) dari Source/Work — sama seperti cover
 * nanovest-calendar dan nanovest-limit-order — sedangkan galeri di
 * bawahnya tetap tiga tangkapan layar ponsel potret 375×812 dari
 * Source/Nanovest Homescreen (--phone, lihat <Media>).
 */
...
export default function NanovestInvestmentApp({
  dict,
  lang,
  data,
}: CaseProps<Cases["nanovest-investment-app"]>) {
  const home = `/${lang}`;
  ...
  return (
    <>
      ...
      <section className="section section--divided tagline-section">
      ...
      {/* §6.4 — markup showreel. Beda dengan NanovestCalendar: `src`
          kosong di sini menyembunyikan section-nya SELURUHNYA,
          bukan menampilkan placeholder — rekamannya belum ada sama
          sekali untuk case ini, jadi poster kosong yang dibingkai
          justru menjanjikan sesuatu yang belum siap. Section muncul
          kembali otomatis begitu `video.src` diisi. */}
      {data.video.src ? (
        <section className="section section--divided case-video" aria-labelledby="case-video-label">
          <div className="section-head reveal">
      ...
      <section className="section section--divided">
        ...
        {/* Baris atas dark, baris bawah light (lihat gallery.items di
            content/*.json — 4+4 berurutan) — dipisah oleh satu
            divider sungguhan (grid-column: 1/-1), bukan caption per
            layar: nama layarnya sudah terbaca dari isi screenshot
            sendiri. cap tetap dipakai sebagai alt text, bukan teks
            yang terlihat. */}
        <div className="case-gallery case-gallery--phones case-gallery--grid4">
          {data.gallery.items.slice(0, 4).map((g) => (
            <Figure key={g.img} img={g.img} ar={g.ar} alt={g.cap || data.title} />
          ))}
          <div className="case-gallery--grid4-divider" aria-hidden="true" />
          {data.gallery.items.slice(4).map((g) => (
            <Figure key={g.img} img={g.img} ar={g.ar} alt={g.cap || data.title} />
          ))}
        </div>
      </section>
      ...
      <section className="section case-next-section">
        ...
        {/* data.next.href path internal ("/work/{slug}"), diprefiks
            `home` (bukan target _blank keluar) karena tautannya
            sesama case internal, bukan ke Framer.
            Sini ("nanovest-investment-app") ada di siklus sebelas case: nanovest-calendar → nanovest-limit-order →
