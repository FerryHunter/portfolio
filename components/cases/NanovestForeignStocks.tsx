...
/**
 * Case study: Nanovest · Foreign Stocks (HK & CN).
 *
 * Satu komponen untuk satu proyek. Susunan section-nya sejajar
 * dengan NanovestCalendar (satu pola untuk dua case Nanovest), dan
 * seluruh asetnya layar ponsel potret 375 × 812 — tidak ada satu pun
 * gambar yang dipotong, yang dibatasi lebarnya (lihat <Media>).
 *
 * Pasangannya di preview statis adalah
 * scripts/case_nanovest_foreign_stocks.py — dua file yang harus tetap
...
export default function NanovestForeignStocks({
  dict,
  lang,
  data,
}: CaseProps<Cases["nanovest-foreign-stocks"]>) {
  const home = `/${lang}`;
  ...
  return (
    <>
      ...
      {/* §6.4 — markup showreel, disembunyikan dulu: rekamannya belum
          ada, jadi section-nya tidak dirender daripada tampil sebagai
          placeholder kosong. Kembalikan begitu `video.src` di
          content/*.json terisi — markup persis di bawah ini, biarkan
          bagian yang di-comment jadi acuannya. */}
      {data.video.src ? (
      ...
      <section className="section section--divided">
        ...
        {/* Delapan layar, 4 kolom eksplisit × 2 baris (case-gallery--grid4):
      ...
      <section className="section case-next-section">
        ...
        {/* data.next.href adalah path internal ("/work/{slug}").
            ...
            nanovest-limit-order → nanovest-foreign-stocks →
