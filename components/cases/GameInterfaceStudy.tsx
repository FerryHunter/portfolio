...
/**
 * Case study: Game Interface Study (Hunters: Legend of Heroes).
 *
 * Susunan section-nya sejajar dengan StiqyDashboard (satu pola
 * hero → overview → scope → video → blocks → gallery → metrics →
 * deliverables → next, tanpa section "anatomi"), dan asetnya juga
 * layar desktop utuh — jadi <Media> di sini memakai `case-media`
 * biasa, bukan `case-media--phone`.
 *
 * Pasangannya di preview statis adalah
 * scripts/case_game_interface_study.py — dua file yang harus tetap
...
export default function GameInterfaceStudy({
  dict,
  lang,
  data,
}: CaseProps<Cases["game-interface-study"]>) {
  const home = `/${lang}`;
  ...
  return (
    <>
      ...
      {/* §6.4 — markup showreel. Belum ada rekaman: section
          disembunyikan dulu daripada menampilkan placeholder kosong.
          Isi data.video.src di content/*.json begitu rekamannya ada,
          section ini muncul lagi otomatis. */}
      {data.video.src ? (
        <section
