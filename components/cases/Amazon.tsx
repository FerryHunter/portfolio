...
/**
 * Case study: Amazon Checkout Redesign (studi konsep, bukan klien).
 *
 * Satu komponen untuk satu proyek. Susunan section-nya sejajar
 * dengan NanovestLimitOrder, dan seluruh asetnya layar ponsel
 * potret utuh 750 × 1624 — tidak ada satu pun gambar yang dipotong,
 * yang dibatasi lebarnya (lihat <Media>). Empat layar, bukan lima:
 * alamat, opsi pengiriman, metode pembayaran, konfirmasi.
 *
 * Pasangannya di preview statis adalah scripts/case_amazon.py — dua
 * file yang harus tetap seragam, mudah dibaca berdampingan.
 */
...
export default function Amazon({
  ...
  lang,
  data,
}: CaseProps<Cases["amazon"]>) {
  const home = `/${lang}`;

  return (
    <>
      <section className="case-hero" id="top">
        <div className="case-hero-deco case-hero-deco--amazon" aria-hidden="true">
      ...
      {/* §6.4 — markup showreel. Tidak ada rekaman untuk case ini
          sama sekali, jadi section-nya disembunyikan daripada
          menampilkan placeholder — bukan §6.4 biasa (`video.src`
          kosong = poster diredupkan + badge), lihat Nanovest dkk. */}
      {data.video.src ? (
        <section
      ...
      <section className="section case-next-section">
        ...
        {/* data.next.href path internal ("/work/{slug}"), diprefiks
            `home` yang hanya diketahui komponen ini saat render —
            sama seperti NanovestInvestmentApp.
            Sini ("amazon") ada di siklus sebelas case: nanovest-calendar → nanovest-limit-order →
