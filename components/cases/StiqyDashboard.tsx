...
/**
 * Case study: Stiqy · Rewards Dashboard.
 *
 * Susunan section-nya sejajar dengan NanovestLimitOrder (satu pola
 * hero → overview → mockup → scope → video → blocks → gallery →
 * metrics → deliverables → next, tanpa section "anatomi"), tapi
 * asetnya layar desktop utuh — jadi <Media> di sini memakai
 * `case-media` biasa, bukan `case-media--phone`. `mockup` adalah
 * satu foto produk (MacBook shot) di antara overview dan scope,
 * murni visual, tanpa section-head.
 *
 * Pasangannya di preview statis adalah
 * scripts/case_stiqy_dashboard.py — dua file yang harus tetap
...
export default function StiqyDashboard({
  dict,
  lang,
  data,
}: CaseProps<Cases["stiqy-dashboard"]>) {
  const home = `/${lang}`;

  return (
    <>
      <section className="case-hero" id="top">
        <div className="case-hero-deco case-hero-deco--stiqy-dashboard" aria-hidden="true">
      ...
      {/* §6.4 — markup showreel. Section disembunyikan sepenuhnya
          selama `video.src` kosong (rekaman belum ada) daripada
          menampilkan placeholder: menyimpannya di data.json tetap
          membuat section ini muncul lagi otomatis begitu src diisi. */}
      {data.video.src ? (
        <section
