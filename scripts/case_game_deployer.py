#!/usr/bin/env python3
"""
Generator preview statis — case study: Web3 Game Deployer Engine.

Sepasang dengan komponennya di components/cases/GameDeployer.tsx.
Susunan section-nya sama persis dengan case_stiqy_dashboard.py (hero →
overview → mockup → scope → video → blocks → gallery → metrics →
deliverables → next), dan `media()` juga dipinjam dari sana: asetnya
layar desktop utuh, bukan potret ponsel, jadi tidak memakai
`case-media--phone`. `mockup` adalah satu foto produk (Overview
screen di atas MacBook) di antara overview dan scope, dibingkai sama
seperti cover (`case-fig--shot case-fig--rounded-lg`).

"next" menaut lewat next.href di JSON, direwrite case_href() dari
path internal (/work/{slug}) ke nama berkas statis — menutup siklus
balik ke nanovest-calendar.
...
Jalankan:  python3 scripts/case_game_deployer.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/game-deployer.html
"""
import json
import pathlib
import sys
...
<section class="case-hero" id="top">
  <div class="case-hero-deco case-hero-deco--game-deployer" aria-hidden="true"><div class="case-hero-deco-blob"></div></div>
...
# ---------- mockup (satu foto produk di antara overview dan scope) ----------
mockup = f'''
<section class="section">
  {figure(C['mockup'], C['mockup']['alt'], extra="case-fig--shot case-fig--rounded-lg")}
</section>
'''
...
# Section disembunyikan sepenuhnya selagi `video.src` kosong: motion
# untuk case ini belum direkam, dan placeholder "MP4 · pending" di
# tengah case study terasa lebih seperti aset yang hilang daripada
# section yang sengaja belum diisi. Isi `video.src` di
# content/*.json begitu rekamannya ada di public/work/game-deployer/
...
html = shell(
    D, LANG,
    f"{hero}{overview}{mockup}{scope}{video}{blocks}{gallery}{metrics}{deliverables}{nxt}",
    title=C["meta"]["title"],
