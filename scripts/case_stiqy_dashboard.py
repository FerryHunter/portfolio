#!/usr/bin/env python3
"""
Generator preview statis — case study: Stiqy · Rewards Dashboard.

Sepasang dengan komponennya di components/cases/StiqyDashboard.tsx.
Susunan section-nya sama dengan case_nanovest_limit_order.py (hero →
overview → mockup → scope → video → blocks → gallery → metrics →
deliverables → next, tanpa section "anatomi"), tapi `media()` di
sini dipinjam dari case_stiqy_questing.py: asetnya layar desktop
utuh, bukan potret ponsel, jadi tidak memakai `case-media--phone`.
`mockup` adalah satu foto produk (MacBook shot) di antara overview
dan scope, murni visual, tanpa section-head.

"next" menaut lewat next.href di JSON, direwrite case_href() dari
path internal (/work/{slug}) ke nama berkas statis.
Sini ("stiqy-dashboard") ada di siklus sebelas case: nanovest-calendar → nanovest-limit-order →
...
Jalankan:  python3 scripts/case_stiqy_dashboard.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/stiqy-dashboard.html
"""
import json
import pathlib
import sys
...
<section class="case-hero" id="top">
  <div class="case-hero-deco case-hero-deco--stiqy-dashboard" aria-hidden="true"><div class="case-hero-deco-blob"></div></div>
...
# Section disembunyikan sepenuhnya selama `video.src` kosong
# (rekaman belum ada) daripada menampilkan placeholder: mengisi
# `video.src` di content/*.json begitu rekamannya ada di
# public/work/stiqy-dashboard/ membuat section ini muncul lagi
