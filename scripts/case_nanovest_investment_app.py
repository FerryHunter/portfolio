#!/usr/bin/env python3
"""
Generator preview statis — case study: Nanovest · Investment App.

Sepasang dengan components/cases/NanovestInvestmentApp.tsx. Susunan
section-nya sama dengan case_nanovest_calendar.py (hero → overview →
scope → motion → problem/approach → screens → results →
deliverables → next; satu pola untuk dua proyek Nanovest, nol
section "Anatomy" terpisah — itu sudah dilebur ke "Screens" di
kedua case) — bedanya cuma satu:

Kartu "next" menaut ke case nanovest-calendar (bukan
nanovest-limit-order — itu case terpisah, bukan kelanjutan proyek
ini) lewat path relatif turunan LANG, bukan lewat `next.href` di
JSON seperti case lain (yang menaut keluar ke Framer) — di situ
href memang string kosong, cuma title/cat/img yang dipakai dari
data.

Hero (`cover`) memakai komposit lanskap dua ponsel dari Source/Work
— sama seperti cover nanovest-calendar dan nanovest-limit-order,
`case-fig--shot` (1120px) — sedangkan galeri "Screens" di bawahnya
tetap tiga tangkapan layar ponsel potret 375×812 dari Source/Nanovest
Homescreen, `case-fig--phone` (lihat `media()`).

Konten dari content/<lang>.json → "cases" → SLUG. Nav, footer,
preloader, dan cookie banner dari scripts/chrome.py.

Jalankan:  python3 scripts/case_nanovest_investment_app.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/nanovest-investment-app.html
"""
import json
import pathlib
import sys
...
def media(item, alt="", eager=False):
    ...
    Sama seperti case_nanovest_calendar.py: slotnya selalu
    `case-media--phone`, dipakai baik untuk komposit lanskap cover
    (tanpa clip-path/scale, transparan — cocok untuk aset yang sudah
    punya latarnya sendiri) maupun tiga tangkapan layar ponsel
    potret 375×812 di galeri. Ukuran akhirnya diatur kelas di
    <figure> pembungkusnya (--shot 1120px vs --phone 380px).
    """
    ar = item["ar"]
...
def figure(item, alt="", extra="", eager=False, show_cap=True):
    cap = item.get("cap") or item.get("caption") or ""
    cap_html = f'<figcaption class="t-meta case-cap">{cap}</figcaption>' if cap and show_cap else ""
...
overview = f'''
<section class="section section--divided tagline-section">
...
# ---------- galeri ----------
# Baris atas dark, baris bawah light (gallery.items di content/*.json
# selalu 4+4 berurutan) — dipisah satu divider sungguhan
# (grid-column: 1/-1), bukan caption per layar: nama layarnya sudah
# terbaca dari isi screenshot sendiri. cap tetap dipakai sebagai alt
# text lewat show_cap=False, bukan teks yang terlihat.
gallery_dark = "".join(
    figure(g, g.get("cap") or C["title"], show_cap=False)
    for g in C["gallery"]["items"][:4])
gallery_light = "".join(
    figure(g, g.get("cap") or C["title"], show_cap=False)
    for g in C["gallery"]["items"][4:])
...
<section class="section section--divided">
  ...
  <div class="case-gallery case-gallery--phones case-gallery--grid4">
    {gallery_dark}
    <div class="case-gallery--grid4-divider" aria-hidden="true"></div>
    {gallery_light}
  </div>
</section>
'''
...
# Beda dengan case_nanovest_calendar.py: `src` kosong di sini
# menyembunyikan section-nya SELURUHNYA (video = "", tidak ikut
# masuk ke concat di bawah), bukan menampilkan placeholder —
# rekamannya belum ada sama sekali untuk case ini, jadi poster
# kosong yang dibingkai justru menjanjikan sesuatu yang belum siap.
# Section muncul kembali otomatis begitu `video.src` diisi.
V = C["video"]
...
# dari path internal (/work/{slug}) ke nama berkas statis. Dulu
# di-hardcode ke nanovest-calendar.html karena next.href masih
# kosong; sekarang datanya sudah diisi, jadi tautan ini otomatis
# ikut kalau siklus "next case" di content/*.json berubah.
n = C["next"]
nxt = f'''
