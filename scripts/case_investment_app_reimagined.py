#!/usr/bin/env python3
"""
Generator preview statis — case study: Investment App Reimagined.

Studi konsep (bukan proyek klien), sepasang gayanya dengan
case_amazon.py: susunan section-nya diturunkan dari
case_nanovest_limit_order.py — itu yang diminta sebagai cetakan —
minus section "mockup" promosi (case ini tidak punya aset komposit
dua-ponsel terpisah, hanya empat tangkapan layar utuh + satu cover),
sama seperti case Amazon yang juga melewatkan section itu.

Empat tangkapan layar sumbernya potret utuh (750 × 1624), disimpan di
public/work/investment-app-reimagined/ pada 375 × 812 (1×) supaya
sama persis dengan konvensi aset ponsel case lain — lihat `media()`,
tidak ada satu pun gambar yang dipotong, yang dibatasi adalah
lebarnya.
...
Jalankan:  python3 scripts/case_investment_app_reimagined.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/investment-app-reimagined.html
"""
import json
import pathlib
import sys
...
SLUG = "investment-app-reimagined"
...
# Tidak ada rekaman untuk case ini, jadi section-nya dilewatkan sama
# sekali daripada tampil sebagai placeholder kosong — sama seperti
# case Amazon. Isi `video.src` di content/*.json begitu rekamannya
# ada di public/work/investment-app-reimagined/, lalu regenerate.
V = C["video"]
...
html = shell(
    D, LANG,
    f"{hero}{overview}{scope}{blocks}{gallery}{video}{metrics}{deliverables}{nxt}",
    title=C["meta"]["title"],
