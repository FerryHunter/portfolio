#!/usr/bin/env python3
"""
Generator preview statis — case study: Nanovest · Event Calendar.

Sepasang dengan komponennya di components/cases/NanovestCalendar.tsx,
dan sengaja berdiri sendiri di sebelah scripts/case_stiqy_questing.py:
susunan section-nya mirip, tapi asetnya berbeda jenis dan itulah yang
mengubah cara gambarnya dipasang.

Perbedaan satu-satunya yang berarti dengan case Stiqy: semua aset di
sini layar ponsel potret 780 × 1688 (9:19,5), bukan tangkapan layar
desktop — jadi tidak ada satu pun gambar yang dipotong, dan yang
dibatasi adalah lebarnya (lihat `media()` dan §6.11).
...
Jalankan:  python3 scripts/case_nanovest_calendar.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/nanovest-calendar.html
"""
import json
import pathlib
import sys
...
def media(item, alt="", eager=False):
    ...
    `case-media--phone`: rasio aselinya sendiri, nol pemotongan,
    dan tanpa latar — aset ponsel sudah membawa sudut membulatnya,
    dan latar slot akan terlihat sebagai empat siku abu-abu di
    ujungnya. Yang mengatur ukurannya CSS, lewat kelas di elemen
    <figure>/kolomnya (§6.11).
...
# ---------- mockup (antara overview dan scope) ----------
mockup = '''
<figure class="case-fig reveal case-fig--wide">
  <div class="case-media" style="--ar:1.3333">
    <img src="../public/work/nanovest-calendar/event-calendar-mockup.webp"
...
# ---------- galeri ----------
# Kolom sempit (case-gallery--phones): tiga layar potret di dua
# kolom lebar penuh akan berdiri setinggi 1.200px masing-masing.
...
# rekamannya ada di public/work/nanovest-calendar/ — markup-nya
...
html = shell(
    D, LANG,
    f"{hero}{overview}{mockup}{scope}{video if V['src'] else ''}{blocks}{gallery}{metrics}{deliverables}{nxt}",
    title=C["meta"]["title"],
