#!/usr/bin/env python3
"""
Generator preview statis — case study: Amazon Checkout Redesign.

Sepasang dengan komponennya di components/cases/Amazon.tsx. Susunan
section-nya sama dengan case_nanovest_limit_order.py — itu yang
dipakai sebagai cetakan — dan aset di sini juga layar ponsel potret
utuh (750 × 1624), jadi tidak ada satu pun gambar yang dipotong, yang
dibatasi adalah lebarnya (lihat `media()`).

Studi konsep (bukan proyek klien): empat layar checkout — alamat,
opsi pengiriman, metode pembayaran, konfirmasi — jadi tidak ada lima
layar seperti case Nanovest, dan section video memakai
order-confirmation.png sebagai poster placeholder.
...
Jalankan:  python3 scripts/case_amazon.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/amazon.html
"""
import json
import pathlib
import sys
...
<section class="case-hero" id="top">
  <div class="case-hero-deco case-hero-deco--amazon" aria-hidden="true"><div class="case-hero-deco-blob"></div></div>
...
# Tidak ada rekaman untuk case ini sama sekali, jadi section-nya
# disembunyikan daripada menampilkan placeholder — bukan §6.4 biasa
# (`video.src` kosong = poster diredupkan + badge), lihat Nanovest
# dkk. Isi `video.src` di content/*.json begitu rekamannya ada di
# public/work/amazon/, lalu regenerate: section-nya muncul sendiri.
V = C["video"]
