#!/usr/bin/env python3
"""
Generator preview statis — case study: Nanovest · Foreign Stocks (HK & CN).

Sepasang dengan komponennya di components/cases/NanovestForeignStocks.tsx.
Susunan section-nya sama dengan case_nanovest_calendar.py — itu yang
dipakai sebagai cetakan — dan aset di sini juga layar ponsel potret
utuh (375 × 812), jadi tidak ada satu pun gambar yang dipotong, yang
dibatasi adalah lebarnya (lihat `media()`).

"next" menaut lewat next.href di JSON, direwrite case_href() dari
path internal (/work/{slug}) ke nama berkas statis.
Sini ("nanovest-foreign-stocks") ada di siklus dua belas case: nanovest-calendar →
nanovest-limit-order → nanovest-foreign-stocks → nanovest-us-stocks-ipo →
crypto-locked-staking → nanovest-investment-app → amazon →
game-interface-study → game-quest → rampage-evolution-card → hris-dashboard → stiqy-dashboard →
...
Jalankan:  python3 scripts/case_nanovest_foreign_stocks.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/nanovest-foreign-stocks.html
"""
import json
import pathlib
import sys
...
# ---------- galeri ----------
# Delapan layar, 4 kolom eksplisit x 2 baris (case-gallery--grid4):
# satu alur penuh, bukan potongan sebagian seperti case Nanovest
# lain — jadi tidak ada divider dark/light di tengahnya.
...
<section class="section section--divided">
  ...
  <div class="case-gallery case-gallery--phones case-gallery--grid4">{gallery_items}</div>
</section>
'''
