#!/usr/bin/env python3
"""
Generator preview statis — case study: Rampage Quest · Daily Quests HUD.

Satu file per proyek, sepasang dengan komponennya di
components/cases/GameQuest.tsx. Susunan section-nya sejajar dengan
case_nanovest_limit_order.py (itu yang dipakai sebagai cetakan), tapi
asetnya layar landscape 3120 × 1440, bukan potret ponsel — jadi
`media()` memakai slot `.case-media` biasa (dipinjam dari
case_stiqy_questing.py), dan galeri memakai `.case-fig--wide` supaya
tiap layar duduk selebar penuh, bukan diperas jadi dua kolom.

"next" menaut lewat next.href di JSON, direwrite case_href() dari
path internal (/work/{slug}) ke nama berkas statis.
Sini ("game-quest") ada di siklus sebelas case: nanovest-calendar → nanovest-limit-order →
...
Jalankan:  python3 scripts/case_game_quest.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/game-quest.html
"""
import json
import pathlib
import sys
...
def media(item, alt="", eager=False):
    ...
    Aset di case ini adalah tangkapan layar landscape utuh (HUD
    game), jadi memakai `.case-media` biasa (bukan varian `--phone`
    di case Nanovest): latar dan clip-path bawaannya pas untuk
    screenshot lebar, tidak untuk layar ponsel potret.
...
<section class="case-hero" id="top">
  <div class="case-hero-deco case-hero-deco--game-quest" aria-hidden="true"><div class="case-hero-deco-blob"></div></div>
...
# rekamannya ada di public/work/game-quest/ — markup-nya berubah
# sendiri, dan motion/reel.js yang meng-attach video saat frame
# mendekati viewport.
V = C["video"]
