#!/usr/bin/env python3
"""
Generator preview statis — case study: Rampage Evolution Card · level-up card system.

Satu file per proyek, sepasang dengan komponennya di
components/cases/RampageEvolutionCard.tsx. Susunan section-nya
sejajar dengan case_game_quest.py (itu yang dipakai sebagai
cetakan) — proyek lain dari game yang sama, Rampage — dan asetnya
juga tangkapan layar landscape 3120 × 1440, bukan potret ponsel,
jadi `media()` memakai slot `.case-media` biasa, bukan varian
`--phone`.

"next" menaut lewat next.href di JSON, direwrite case_href() dari
path internal (/work/{slug}) ke nama berkas statis.
Sini ("rampage-evolution-card") disisipkan di siklus dua belas
case, tepat setelah game-quest — sesama proyek Rampage:
nanovest-calendar → nanovest-limit-order → nanovest-us-stocks-ipo →
crypto-locked-staking → nanovest-investment-app → amazon →
game-interface-study → game-quest → rampage-evolution-card →
hris-dashboard → stiqy-dashboard → game-deployer →
...
Jalankan:  python3 scripts/case_rampage_evolution_card.py [lang]
Sajikan :  python3 -m http.server 4321   →  /preview/rampage-evolution-card.html
"""
import json
import pathlib
import sys
...
def media(item, alt="", eager=False):
    ...
    Aset di case ini adalah tangkapan layar landscape utuh (layar
    upgrade kartu), jadi memakai `.case-media` biasa (bukan varian
    `--phone` di case Nanovest): latar dan clip-path bawaannya pas
    untuk screenshot lebar, tidak untuk layar ponsel potret.
...
<section class="case-hero" id="top">
  <div class="case-hero-deco case-hero-deco--rampage-evolution-card" aria-hidden="true"><div class="case-hero-deco-blob"></div></div>
...
# ---------- galeri ----------
# Grid 2×2, bukan case-fig--wide: empat layar berdampingan dua-dua
# terbaca lebih cepat sebagai satu set state daripada ditumpuk satu
# kolom penuh per baris.
...
# Dihitung tapi TIDAK disertakan di `html` di bawah: rekamannya
# belum ada, jadi section-nya disembunyikan dulu daripada tampil
# sebagai placeholder kosong. Masukkan lagi ke perakitan `html`
# begitu `video.src` di content/*.json terisi.
V = C["video"]
...
html = shell(
    D, LANG,
    # `video` sengaja tidak disertakan: rekamannya belum ada
    # (V["src"] kosong), dan placeholder MP4 lebih baik disembunyikan
    # dulu daripada menampilkan section kosong. Kode di atas tetap
    # dihitung supaya tinggal ditambahkan kembali ke sini begitu
    # rekamannya siap.
